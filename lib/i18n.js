...
const strings = {
  ...
  verify_title:     { fr: 'Verifiez votre numero',        wo: 'Wóoral sa limu telefon' },
  verify_why:       { fr: "Pour deposer une annonce, nous devons verifier que ce numero est bien le votre. C'est ce qui protege le site des arnaqueurs.", wo: 'Ngir génne ab annonce, war nanu xam ne limu bi sa moom la. Loolu mooy aar site bi ci naaféq yi.' },
  verify_send:      { fr: 'Recevoir mon code par SMS',    wo: 'Yónnee ma kode bi ci SMS' },
  verify_resend:    { fr: 'Renvoyer le code',             wo: 'Delloo kode bi' },
  verify_sent_to:   { fr: 'Un code a 6 chiffres a ete envoye au', wo: 'Kode bu am 6 chiffre dem na ci' },
  verify_code:      { fr: 'Code recu par SMS',            wo: 'Kode bi nga jot ci SMS' },
  verify_submit:    { fr: 'Valider mon numero',           wo: 'Wóoral limu bi' },
  verify_ok:        { fr: 'Numero verifie ! Vous pouvez maintenant deposer vos annonces.', wo: 'Limu bi wóor na ! Man nga leegi génne say annonce.' },
  verify_badge:     { fr: 'Numero verifie',               wo: 'Limu bu wóor' },
  verify_needed:    { fr: 'Verifiez votre numero pour deposer une annonce.', wo: 'Wóoral sa limu ngir génne ab annonce.' },
  verify_no_sms:    { fr: "Vous n'avez rien recu ? Verifiez le reseau, puis demandez un nouveau code apres une minute.", wo: 'Jotoo dara ? Seetal reseau bi, te laaj beneen kode ci ginnaaw benn simili.' },
  verify_test_mode: { fr: 'Mode test : votre code est',   wo: 'Mode test : sa kode mooy' },
  verify_later:     { fr: 'Plus tard',                    wo: 'Ci kanam' },

  err_otp_wait:            { fr: 'Patientez un instant avant de redemander un code.', wo: 'Xaaral tuuti bala nga laaj beneen kode.' },
  err_otp_too_many_sends:  { fr: 'Trop de codes demandes. Reessayez dans une heure.', wo: 'Laaj nga kode lu bare. Jéemaat ci benn waxtu.' },
  err_otp_send_failed:     { fr: "L'envoi du SMS a echoue. Reessayez dans quelques minutes.", wo: 'SMS bi demul. Jéemaat ci ay simili.' },
  err_otp_none:            { fr: 'Demandez d abord un code.', wo: 'Laajal kode bi bu njëkk.' },
  err_otp_expired:         { fr: 'Ce code a expire. Demandez-en un nouveau.', wo: 'Kode bi jeex na. Laajal bu bees.' },
  err_otp_wrong:           { fr: 'Code incorrect.',       wo: 'Kode bi baaxul.' },
  err_otp_tries_left:      { fr: 'essai(s) restant(s)',   wo: 'jéem yu des' },
  err_otp_too_many_tries:  { fr: 'Trop d essais. Demandez un nouveau code.', wo: 'Jéem nga lu bare. Laajal beneen kode.' },
  ...
  safety_5:         { fr: "Un numero verifie n'est pas une garantie : restez prudent malgre tout.", wo: 'Limu bu wóor du wóoral lépp : moytul ba tey.' },
